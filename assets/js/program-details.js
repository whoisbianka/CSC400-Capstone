(() => {
  ...
  const text = (value, fallback = 'Not available in the current dataset.') =>
  ...
  function library() {
    ...
    const records = window.programCatalog
      .getPrograms()
      .filter(p => [p.major, p.college, p.state].join(' ').toLowerCase().includes(query));
    $('library-count').textContent =
      `${records.length} programs` +
      (window.programCatalog.isDemo() ? ' · Fictional demo data' : '');
    $('library-results').replaceChildren();
    records.forEach(program => {
      const link = document.createElement('a');
      link.className = 'library-card';
      link.href =
        '#' + encodeURIComponent(JSON.stringify({ program, demo: window.programCatalog.isDemo() }));
      const title = document.createElement('h2');
      title.textContent = program.major;
      const college = document.createElement('p');
      college.textContent = program.college;
      const meta = document.createElement('p');
      meta.textContent = [program.state, program.format].filter(Boolean).join(' · ');
      link.append(title, college, meta);
      $('library-results').append(link);
    });
    if (!records.length) {
      const empty = document.createElement('p');
      empty.textContent = 'No matching programs. Try another major or college.';
      $('library-results').append(empty);
    }
  }
  function render() {
    ...
    if (!location.hash) {
      ...
      $('record-label').textContent = 'Your workspace';
      library();
      return;
    }
    try {
      ...
      if (!p || typeof p.major !== 'string' || typeof p.college !== 'string')
      ...
      $('record-notice').textContent = record.demo
        ? 'Demo data only: this college, program, and tuition are fictional test fixtures.'
        : 'Snapshot of the program record selected in chat. Confirm current requirements and costs with the college.';
      const tuition =
        typeof p.tuition === 'number' && Number.isFinite(p.tuition) && p.tuition >= 0
          ? '$' + p.tuition.toLocaleString('en-US')
          : 'Not available';
      [
        ['College', p.college],
        ['Location', p.state],
        ...
        group.append(dt, dd);
        $('program-facts').append(group);
      });
      $('program-overview').textContent = text(
        p.overview,
      ...
      if (typeof p.url === 'string') {
        try {
          ...
          if (['https:', 'http:'].includes(url.protocol)) {
            const link = document.createElement('a');
            link.href = url.href;
            link.textContent = 'Visit program website';
            $('program-website').replaceChildren(link);
          }
        } catch {
          /* Missing or unsafe URLs remain unavailable. */
        }
      }
      $('program-content').hidden = false;
    } catch {
      $('program-title').textContent = 'Program unavailable';
      $('college-name').textContent =
        'This link is incomplete or invalid. Return to chat and open a matching program.';
    }
  }
  $('program-search').addEventListener('input', library);
  window.addEventListener('hashchange', () => {
    render();
    document.querySelector('.program-workspace').scrollTop = 0;
    $('program-title').setAttribute('tabindex', '-1');
    $('program-title').focus();
  });
