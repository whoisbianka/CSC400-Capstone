(() => {
  'use strict';
  window.createProgramTable = (container, options = {}) => {
    const $ = id => container.querySelector('#' + id);
    const getPrograms = options.getPrograms || (() => window.programCatalog.getPrograms());
    const columns = [
      ['college', 'College'],
      ['state', 'Location'],
      ['major', 'Major'],
    ...
    function render() {
      ...
      columns.forEach(([key, label]) => {
        ...
        button.addEventListener('click', () => {
          ...
          render();
          $('tbl-head')
            .children[columns.findIndex(([column]) => column === key)].querySelector('button')
            .focus();
        });
        th.append(button);
        $('tbl-head').append(th);
      ...
      rows.forEach(program => {
        ...
        columns.forEach(([key]) => {
          const td = node(
            'td',
            key === 'matched'
              ? (program.matched || []).join(', ') || 'Matches your stated filters'
              : key === 'tuition'
                ? program[key] === null
                  ? 'Not available'
                  : '$' + program[key].toLocaleString('en-US')
                : program[key] || 'Not available'
          );
          if (key === 'major') {
      ...
      if (!rows.length) {
        const tr = node('tr', ''),
          td = node('td', 'No results match your search or filters.');
      ...
      $('catalog-note').textContent = window.programCatalog.isDemo()
        ? 'Demo data only · All colleges, programs, and tuition figures are fictional test fixtures.'
        : 'Program catalog · Confirm requirements and costs with the college.';
      $('tbl-filters')
    ...
    function refresh() {
      ...
      ['', ...formats].forEach(value => {
        ...
        button.type = 'button';
        button.className = 'tbl-chip';
        button.dataset.format = value;
        button.addEventListener('click', () => {
          format = value;
          render();
        });
        $('tbl-filters').append(button);
      });
      render();
    }
