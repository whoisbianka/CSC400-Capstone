(() => {
  ...
  function renderResults(result, target = $('messages')) {
    ...
    const author = node(
      'p',
      result.demo
        ? 'Degree Path Assistant · Demo results'
        : 'Degree Path Assistant · Program results'
    );
    author.className = 'message-author';
    ...
    const disclaimer = node(
      'p',
      result.demo
        ? 'Fictional test data — not real colleges, rankings, costs, or recommendations.'
        : 'Results from the connected program catalog. Check program requirements and costs with the college.'
    );
    disclaimer.className = 'demo-disclaimer';
    item.append(disclaimer);
    ...
    if (result.rows.length) {
      ...
      const count = node('p', `${colleges.size} colleges · ${result.rows.length} related programs`);
      ...
      colleges.forEach(programs => {
        ...
        const tuition = costs.length
          ...
          : 'Not available';
        [
          ['Annual tuition' + (result.demo ? ' · demo' : ''), tuition],
          [
            'Study format',
            [...new Set(programs.map(p => p.format).filter(Boolean))].join(' / ') || 'Not available'
          ]
        ].forEach(([label, value]) => {
  ...
  function showAsk(focus = false) {
    if (mode === 'guided' && current !== null && !$('chat-form').hidden)
      guidedDraft = { index: current, editing, text: $('chat-message').value };
    activate('ask');
    current = null;
    ...
    queryMessages.forEach(message =>
      message.result ? renderResults(message.result) : bubble(message.text, message.user)
    );
  ...
  function startGuided() {
    ...
    if (guidedDraft) {
      ...
      if (next === -1) review();
      else showQuestion(next, false, true);
    }
  }
  function showQuestion(index, isEdit = false, focus = false) {
    ...
    transcript();
    bubble(
      `${isEdit ? 'Edit answer' : 'Question'} ${index + 1} of ${questions.length}: ${questions[index].text}`
    );
  ...
  $('chat-message').addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
      e.preventDefault();
      if ($('chat-message').value.trim()) $('chat-form').requestSubmit();
    }
  });
  ...
  function refreshSuggestions() {
    ...
    if (!suggestions.length)
      container.append(
        node('p', 'Suggested questions will appear when program data is available.')
      );
  }
